...
*some* detour is structurally forced — the least-squares tracker
([`run_geodesic_tracker.py`](../scripts/run_geodesic_tracker.py)) realizes only 62–79% of the
required velocity direction, and that deficit is a statement about operator spans rather than
about any optimizer.

> **⚠ Retracted claim.** An earlier version of this paragraph went further and called
> `D_path` ≈ 0.45 the family's **intrinsic** distance from time-optimality, "not slack an
> optimizer can remove". **That was wrong.** Putting the geodesic into the objective and
> sweeping its weight ([`run_pareto.py`](../scripts/run_pareto.py)) reaches
> **`D_path` = 0.155 at F = 0.992** — three times closer, for eight tenths of a percent of
> fidelity. The 0.45 measures what a *fidelity-only* objective happens to produce, not what
> the family can reach. The velocity deficit above is structural; the 0.45 was not. See
> `docs/paper/main.tex`, "Reachability of the pump family under a
> geodesic-aware objective".
...
> **⚠ Revised twice — read this whole block.** This section originally concluded that
> gate-based control "has a floor near 1.0 rad". Every number above comes from a
> **fidelity-only** objective, and that reasoning already failed once for pulses (see the
> retraction earlier in this file). Under a geodesic-aware objective
> ([`run_gate_pareto.py`](../scripts/run_gate_pareto.py)) both gate sets go lower, so 1.03 is
> not a floor.
>
> A first revision then claimed the real result was an *asymptote* shared by both
> instruction sets at `D_path` ≈ 0.7. **That was also wrong**, and for the same reason a
> third time: it was measured at one depth per family (4 layers for displacement+SNAP, 16
> for ECD). Sweeping depth (`figures/depth/`) shows displacement+SNAP falls monotonically —
> **0.660 (4 layers) → 0.397 (8) → 0.278 (12)**, still falling — while ECD does not improve
> (0.626 at 16 layers, 0.762 at 24). There is no shared floor and no established floor at
> all; the deepest circuit tested is still descending.
>
> The durable lesson is methodological: on this problem, *any* `D_path` measured at fixed
> objective and fixed depth is an upper bound on what the family can reach, not a property
> of the family. Three separate claims in this file were made by forgetting that.
